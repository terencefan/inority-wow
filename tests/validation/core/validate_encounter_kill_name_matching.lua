...
assert(EncounterState.IsEncounterKilledByName(state, "死亡") == false, "expected partial boss name not to inherit another boss's kill state")
assert(EncounterState.IsEncounterKilledByName(state, "使者") == false, "expected suffix-only partial boss name not to inherit another boss's kill state")
