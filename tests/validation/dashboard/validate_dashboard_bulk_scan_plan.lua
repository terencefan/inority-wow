...
assert(clearedSet["raid::军团再临"], "expected legion expansion clear")
assert(clearedSet["raid::德拉诺之王"], "expected draenor expansion clear")
assert(refreshCalls == 1, "expected plan preparation to refresh dashboard once")
assert(addon.dashboardBulkScanState == nil, "expected top-level scan button to prepare plan without starting active scan")
