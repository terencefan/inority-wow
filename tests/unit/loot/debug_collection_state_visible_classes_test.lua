...
local function assertListEquals(actual, expected, label)
	...
	for index, value in ipairs(expected) do
		assert(actual[index] == value, string.format("%s[%d] mismatch: %s ~= %s", label, index, tostring(actual[index]), tostring(value)))
	end
end

local leatherVisible = getVisibleSelectedLootClasses(
	function()
...
local clothVisible = getVisibleSelectedLootClasses(
	function()
...
local noSelectionVisible = getVisibleSelectedLootClasses(
	function()
		return { "PRIEST", "MAGE", "WARLOCK" }
	end,
	function()
