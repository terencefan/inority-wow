...
EventsCommandController.Configure({
	...
	InitializeDefaults = function() end,
	PruneExpiredBossKillCaches = function() end,
	CaptureSavedInstances = function() end,
	InitializePanel = function() end,
	InitializeLootPanel = function() end,
	CreateMinimapButton = function() end,
	InvalidateLootDataCache = function() end,
	RecordEncounterKill = function() end,
	RefreshLootPanel = function() end,
})

EventsCommandController.RegisterCoreEvents(frame, "MogTracker")

assert(#registeredEvents == 6, "expected six registered core events")
...
assert(
	printed[3]
		== "event: ENCOUNTER_LOOT_RECEIVED encounterID=2032 itemID=19019 item=|cff0070dd|Hitem:19019::::::::|h[雷霆之怒]|h|r",
	"expected encounter loot received message"
)
assert(printed[4] == "event: ENCOUNTER_END boss=Illidan Stormrage success=1", "expected encounter end message")
assert(printed[5] == "event: GET_ITEM_INFO_RECEIVED count=2 window=1s", "expected aggregated item info message")
assert(printed[6] == "event: UPDATE_INSTANCE_INFO count=2 window=1s", "expected aggregated instance info message")
