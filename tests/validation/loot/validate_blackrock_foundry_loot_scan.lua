...
for _, encounter in ipairs(data.encounters or {}) do
  assert(#(encounter.loot or {}) == 1, "expected encounter-level loot enumeration to populate one row per fixture encounter")
end
...
assert(firstFilterRun.totalLoot == #fixture.journal_encounters, "expected totalLoot to come from encounter-level aggregation")
assert(fixture.loot_filter_debug.totalLootByClassIDBeforeFix[5] == 0, "expected fixture to record the pre-fix zero-loot symptom")
