...
local function assertSetEquals(actualValues, expectedValues, context)
  ...
  for key in pairs(actualSet) do
    assert(expectedSet[key], string.format("expected %s to exclude unexpected value %s", tostring(context), tostring(key)))
  end
end
...
assert(resolvedOrder[1] == 16 and resolvedOrder[2] == 15 and resolvedOrder[3] == 14 and resolvedOrder[4] == 17, "expected stable difficulty display order")
...
assert(observedByDifficultyID[15] == false and observedByDifficultyID[14] == false, "expected unobserved heroic/normal markers")
...
for _, item in ipairs(acceptedItems) do
  ...
  selectedCountByEncounterName[tostring(item.encounterName)] = (selectedCountByEncounterName[tostring(item.encounterName)] or 0) + 1
  assert(type(item.selectedVisibleClasses) == "table" and #item.selectedVisibleClasses == 1, "expected one visible selected class per accepted item")
...
for _, boss in ipairs(bossLootCounts) do
  ...
  assert(selectedCount == expectedSelectedCount, string.format("expected selectedCount %d for %s", expectedSelectedCount, encounterName))
  ...
  if boss.allCountKnown == true then
    ...
    assert(allCount ~= nil, string.format("expected numeric allCount when allCountKnown=true for %s", encounterName))
    ...
    assert(boss.allCount == nil, string.format("expected nil allCount when allCountKnown=false for %s", encounterName))
  end
end
...
assert(tonumber(lootPanelAssertions.bossCountSummary.bossesWithZeroSelectedLoot) == 6, "expected boss zero-selected summary")
assert(tonumber(lootPanelAssertions.bossCountSummary.bossesWithKnownAllCounts) == 0, "expected boss known-all summary")
assert(tonumber(lootPanelAssertions.bossCountSummary.bossesWithImpossibleZeroZero) == 0, "expected boss zero-zero summary")
...
assertSetEquals(
  bulkScanAssertions.matchingSelectionKeys,
  {
    "current",
    "1195::初诞者圣墓::16",
    "1195::初诞者圣墓::15",
...
assertSetEquals(
  bulkScanAssertions.matchingRaidQueueKeys,
  {
    "1195::初诞者圣墓::16",
    "1195::初诞者圣墓::15",
    "1195::初诞者圣墓::14",
